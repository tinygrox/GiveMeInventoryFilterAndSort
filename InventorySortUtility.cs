...
public static class InventorySortUtility
{
    ...
    public static List<Item> MergeSortedItems_Hashed_Queue(List<Item> items, out List<Item> itemsToDestroy)
    {
        ...
        Dictionary<(int TypeID, string FromInfoKey), Queue<Item>> openQueues = new();
        List<Item> finalResult = new();
        foreach (Item current in items)
        {
            if (!current || current.StackCount <= 0)
            {
                continue;
            }
            ...
            (int TypeID, string FromInfoKey) currentItemMergeKey = (current.TypeID, current.FromInfoKey);

            // if (openQueues.TryGetValue(current.TypeID, out var queue))
            if (openQueues.TryGetValue(currentItemMergeKey, out Queue<Item> queue))
            {
                while (queue.Count > 0 && current.StackCount > 0)
                {
                    Item target = queue.Peek();
                    ...
                    if (target.StackCount >= target.MaxStackCount)
                    {
                        queue.Dequeue();
                    }
                }
                ...
                if (queue.Count == 0)
                {
                    openQueues.Remove(currentItemMergeKey);
                }
            }
            ...
            if (current.StackCount > 0)
            {
                ...
                if (current.StackCount < current.MaxStackCount)
                {
                    ...
                    if (!openQueues.TryGetValue(currentItemMergeKey, out Queue<Item> q2))
                    {
                        q2 = new Queue<Item>();
    ...
    {
        SortConfigurator configurator = new(itemsToSort);
    ...
    public static void SortInventoryByCriteria(InventoryDisplay inventoryToSort, Action<SortConfigurator> configureSort)
    {
        ...
        try
        {
            ...
            Queue<Item> finalItemQueue = new(sortedAndMergedItems);
