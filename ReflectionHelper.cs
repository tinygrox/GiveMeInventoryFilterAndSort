...
namespace tinygrox.DuckovMods.GiveMeInventoryFilter
{
    public static class ReflectionHelper
    {
        ...
        public static Func<TInstance, TField> CreateFieldGetter<TInstance, TField>(string fieldName)
        {
            ...
            if (s_getterCache.TryGetValue(key, out var getter))
            {
            ...
            try
            {
                ...
                var newGetter = Expression.Lambda<Func<TInstance, TField>>(body, instanceParam).Compile();
        ...
        {
            var key = (typeof(TInstance), fieldName);
            if (s_setterCache.TryGetValue(key, out var setter))
            {
            ...
            try
            {
                ...
                var newSetter = Expression.Lambda<Action<TInstance, TValue>>(assignExpr, instanceParam, valueParam).Compile();
        ...
        public static Func<TInstance, object[], TReturn> CreateMethodCaller<TInstance, TReturn>(string methodName, params Type[] parameterTypes)
        {
            try
            {
                ...
                {
                    var block = Expression.Block(callExpr, Expression.Default(typeof(TReturn)));
