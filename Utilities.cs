...
public class DropdownScrollbarAutoScroll : MonoBehaviour
{
    ...
    private async UniTask GoToSelected()
    {
        ...
        // 使用浮点数进行除法，避免整数除法问题
        float ratio = selectedOption / (totalOptions - 1f);
...
public class DropOptionsLocalization : MonoBehaviour
{
    ...
    private void OnDestroy() => LocalizationManager.OnSetLanguage -= RefreshDropdownOptions;
...
public static class Utilities
{
    ...
    public static List<string> GetTranslatedOptions() => DropdownOptions.Select(key => key.ToPlainText()).ToList();
    ...
    public static void ExecuteSortByIndex(InventoryDisplay inventoryDisplay, int currentIndex)
    {
        ...
        ModLogger.Log.Debug($"'{currentIndex}' 排序 {inventoryDisplay.Target.Content.Count} 物品花费了 {timer.ElapsedMilliseconds}ms");
    }
    ...
    public static void AddDropdown(Transform inventoryDisplayTransform, InventoryDisplay inventoryDisplay, bool showSpace = true, bool showDropdown = true)
    {
        ...
        HorizontalLayoutGroup titleBar1 = inventoryDisplayTransform.GetComponentInChildren<HorizontalLayoutGroup>();
        if (!titleBar1)
        {
            return;
        }
        ...
        GameObject dropdownObj = parent.Find("Filter_Dropdown")?.gameObject;
        if (dropdownObj)
        {
        ...
        Transform space = parent.transform.Find("space");
        if (space)
        {
        ...
        if (dropdownObj)
        {
            ...
            if (dropdownComp)
            {
                TextMeshProUGUI captionText = dropdownComp.captionText as TextMeshProUGUI;
                if (captionText)
                {
                    captionText.overflowMode = TextOverflowModes.Ellipsis;
                }

                TextMeshProUGUI itemLabel = dropdownComp.itemText as TextMeshProUGUI;
                if (itemLabel)
                {
    ...
    public static void PopulateDropdown(TMP_Dropdown dropdown)
    {
        ...
        TextMeshProUGUI itemLabel = dropdown.itemText as TextMeshProUGUI;
    ...
    private static void DropdownValueChanged(TMP_Dropdown dropdown, InventoryDisplay inventoryDisplay) => ExecuteSortByIndex(inventoryDisplay, dropdown.value);
}
